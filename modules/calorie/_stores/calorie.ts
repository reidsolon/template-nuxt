...
export const useCalorieStore = defineStore('calorie', {
  ...
  getters: {
    // Food Items
    getFoodItemById: (state) => (id: string): FoodItem | undefined => {
    ...
    searchFoodItems: (state) => (query: string) => {
      ...
      return state.foodItems.filter(food => 
    ...
    // Calorie Entries
    getCalorieEntryById: (state) => (id: string): CalorieEntry | undefined => {
      return state.calorieEntries.find(entry => entry.id === id)
    },

    getEntriesWithFood: (state): CalorieEntryWithFood[] => {
      return state.calorieEntries.map(entry => {
    ...
    getEntriesForDate: (state) => (date: Date): CalorieEntryWithFood[] => {
      const { start, end } = getDayRange(date)
      const entriesWithFood = state.calorieEntries.map(entry => {
        const food = state.foodItems.find(f => f.id === entry.foodId)
        if (!food) return null
      ...
      return entriesWithFood.filter(entry => 
        entry.consumedAt >= start && entry.consumedAt <= end
      )
    },

    getEntriesForDateRange: (state) => (startDate: Date, endDate: Date): CalorieEntryWithFood[] => {
      const entriesWithFood = state.calorieEntries.map(entry => {
        const food = state.foodItems.find(f => f.id === entry.foodId)
        if (!food) return null
      ...
      return entriesWithFood.filter(entry => 
        entry.consumedAt >= startDate && entry.consumedAt <= endDate
      )
    },

    // Daily Summary
    getDailySummary: (state) => (date: Date): DailySummary => {
      ...
      const mealBreakdown = entries.reduce((breakdown, entry) => {
    ...
    getCalorieProgress: (state) => (date: Date): CalorieProgress => {
      const entries = state.calorieEntries
        .filter(entry => isSameDay(entry.consumedAt, date))
        .map(entry => {
          const food = state.foodItems.find(f => f.id === entry.foodId)
