...
export const useCalorieFilters = () => {
  ...
  const filterEntries = (
    ...
    if (filter.dateFrom) {
      filteredEntries = filteredEntries.filter(entry => 
        entry.consumedAt >= filter.dateFrom!
      )
    }
    ...
    if (filter.mealType) {
      filteredEntries = filteredEntries.filter(entry => 
        entry.mealType === filter.mealType
      )
    }
    ...
    if (filter.category) {
      filteredEntries = filteredEntries.filter(entry => 
        entry.food.category === filter.category
      )
    }
  ...
  const searchFoodItems = (query: string): FoodItem[] => {
    ...
    return store.foodItems.filter(food => 
  ...
  const getFoodSuggestions = (limit: number = 5): FoodItem[] => {
    ...
    return sortedFoods
      .map(([foodId]) => store.getFoodItemById(foodId))
