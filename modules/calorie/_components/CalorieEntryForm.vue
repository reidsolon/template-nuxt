    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="space-y-2">
        ...
        <div class="relative">
          ...
          >
            ...
            >
              No foods found.
              <button type="button" @click="showAddFoodModal = true" class="text-blue-600 hover:text-blue-800">
                Add custom food
              </button>
            </div>
            ...
            >
              <div class="flex justify-between items-start">
                <div>
                  ...
                  <div class="text-sm text-gray-500">{{ food.brand }} • {{ food.category }}</div>
      ...
      <div v-if="selectedFood" class="p-3 bg-blue-50 rounded-md">
        <div class="flex justify-between items-start">
          ...
          <button type="button" @click="clearSelectedFood" class="text-gray-400 hover:text-gray-600">
      ...
      <!-- Meal Type -->
      <BaseSelect v-model="form.mealType" label="Meal Type" required :error="errors.mealType">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <BaseInput v-model="form.time" label="Time" type="time" required :error="errors.time" />
      </div>
      ...
      <div v-if="selectedFood && form.quantity" class="p-3 bg-gray-50 rounded-md">
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            Calories: <span class="font-medium">{{ totalCalories }}</span>
          </div>
          <div v-if="totalNutrition.protein">
            Protein: <span class="font-medium">{{ totalNutrition.protein }}g</span>
          </div>
          <div v-if="totalNutrition.carbs">
            Carbs: <span class="font-medium">{{ totalNutrition.carbs }}g</span>
          </div>
          <div v-if="totalNutrition.fat">
            Fat: <span class="font-medium">{{ totalNutrition.fat }}g</span>
          </div>
        </div>
      </div>
